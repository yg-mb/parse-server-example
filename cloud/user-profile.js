...
function applyProductChange(userProfile, product, amount, params) {
    ...
    switch (product.get("name")) {
        case "max_book_pages":
         	current = userProfile.get("max_page_number") || 50; //default max page number
            userProfile.set("max_page_number", current + amount);
            break;
        case "max_avatars":
         	current = userProfile.get("max_avatar_number") || 3; //default max avatar number
            userProfile.set("max_avatar_number", current + amount);
            break;
        case "max_record_secs":
            current = userProfile.get("max_recording_length") || 10; //default max recording length
            userProfile.set("max_recording_length", current + amount);
            break;
        case "max_custom_assets":
            current = userProfile.get("max_custom_asset_number") || 20; //default max custom asset number
            userProfile.set("max_custom_asset_number", current + amount);
            break;
        case "max_custom_scenes":
            current = userProfile.get("max_custom_scenes_number") || 3; //default max custom scene number
            userProfile.set("max_custom_scenes_number", current + amount);
            break;
        case "max_custom_musics":
            current = userProfile.get("max_custom_musics_number") || 2; //default max custom music number
            userProfile.set("max_custom_musics_number", current + amount);
            break;
        case "max_custom_soundeffect":
            current = userProfile.get("max_custom_soundeffect_number") || 3; //default max custom music number
            userProfile.set("max_custom_soundeffect_number", current + amount);
            break;
        case "unlock_item":
            if (params) {
                current = userProfile.get("unlock_items") || ""; //default unlocked item name
...
function applyProductToUser(userProfileHolder, product, amount, params) {
    ...
    try {
        ...
        var promises = [];
        promises.push(userProfile.save(null, {
            useMasterKey: true
        }));
...
function recordUserPurchaseHistory(userProfile, product, amount, coinsChange, params) {
    ...
    return userPurchaseHistory.save(null, {
        useMasterKey: true
    });
}
...
function refreshUserStats(userProfileHolder, books) {
    ...
    var totalCheats = 0;
    var totalAppUseTimeScore = user.get("timePlayedTotal") / 20 || 0;
...
Parse.Cloud.define("blockUser", function(request, response) {
    ...
    Parse.Promise.when(promises).then(function(results) {
        ...
        .then(function() {
            return logUserAction(blockBy, blocked ? "block" : "unblock", blockMessage);
        })
        .then(function() {
...
function blockUserContent(username, blocked) {
    ...
    return Parse.Promise.when(queryPromises).then(function(results) {
        ...
        for (var i = 0; i < books.length; i++) {
            ...
            updatePromises.push(book.save(null, {
                useMasterKey: true
            }));
        }
        console.log("Updating bookComments:" + bookComments.length);
        for (var j = 0; j < bookComments.length; j++) {
            var bookComment = bookComments[j];
            bookComment.set("active", !blocked);
            updatePromises.push(bookComment.save(null, {
                useMasterKey: true
            }));
        }
